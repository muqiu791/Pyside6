...
from ultralytics.utils import DEFAULT_CFG, LOGGER, SETTINGS, callbacks, ops, colorstr
...
class YoloPredictor(BasePredictor, QObject):
            ...
            try:
                while True:
                    ...
                    if self.stop_dtc:
                        for writer in self.vid_writer.values():
                            if isinstance(writer, cv2.VideoWriter):
                                writer.release()  # 释放 video writer
                        self.vid_writer.clear()  # 清空字典
                    ...
                    if self.continue_dtc:
                        ...
                        # print('type(batch):',type(batch)) # <class 'tuple'>
                        # print('lenth of batch:',len(batch)) # 3


                        paths, im0s, s = batch  # 正确解包前三个元素
                        paths = ' '.join(paths) if isinstance(paths, list) else paths  # 将路径列表转换为字符串
                        s = ' '.join(s) if isinstance(s, list) else s  # 将s列表转换为字符串
                        vid_cap = self.dataset.cap  # 假设vid_cap不在batch中，并将其设置为None或适当的默认值
                        print('vid_cap:',vid_cap) # None

                        im0s = np.array(im0s)  # 将列表转换为numpy数组
                        print('im0s.shape :',im0s.shape) # (1, 640, 640, 3)

                        # Preprocess
                        with self.dt[0]:
                            im = self.preprocess(im0s) # (1, 3, 640, 640)
                        print('im.shape :',im.shape) # (1, 640, 640, 3)

                        visualize = increment_path(self.save_dir / Path(paths).stem, mkdir=True) if self.args.visualize else False
                        ...
                        # with profilers[1]:
                        #     preds = self.inference(im, self.args.augment,visualize)
                            # if self.args.embed:
                            #     yield from [preds] if isinstance(preds,
                            #                                      torch.Tensor) else preds  # yield embedding tensors
                            #     continue
                        ...
                        print('len im', len(im))
                        print('shape im0s', im0s.shape)
                        print('len im0s', len(im0s))
                        for i in range(n):
                            ...
                            if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                                ...
                                self.yolo2main_pre_img.emit(im0s[i] if isinstance(im0s, np.ndarray) else im0s)
                                self.yolo2main_res_img.emit(im0s[i])  # after detection
                                # 发送标签、类别数和目标数信号
                                self.yolo2main_labels.emit(self.labels_dict)


                            print('self.source_type ',self.source_type)
                            # p, im0 = (paths[i], im0s[i].copy()) if self.dataset.mode == "video" or self.source_type.from_img \
                            #     else (paths, im0s.copy())
                            #
                            # # print('im0:',im0.shape) # (640, 640, 3)
                ...
                #
                # print('save results im0',im0.shape)
                # print('save results im0s',im0s.shape)
                # print('im0s.shape[0]',im0s.shape[0])
                # Send test results
                # self.yolo2main_res_img.emit(im0) # after detection
                # self.yolo2main_pre_img.emit(im0s if isinstance(im0s, np.ndarray) else im0s[i])   # Before testing
    ...
    def write_results(self, i, p, im, s):
        ...
        # 计算不同类别的数量和检测到的目标总数
        class_nums = len(np.unique(result.classes))  # 假设 result.classes 包含所有检测到的类别
        target_nums = len(result)  # 假设 result 可以直接用来计算检测到的目标总数
    ...
    # def write_results(self, idx, p, im, s):
    #     # p, im, im0 = batch
    #     # print('result shape of im:',im.shape) # [1, 3, 640, 640])
    #     # im0 = np.stack((im[0][2], im[0][1], im[0][0]), axis=-1)
    #     # print('result shape of im0:',im0.shape) # (640, 640, 3)
    #
    #     log_string = ''
    #     if len(im.shape) == 3:
    #         im = im[None]  # expand for batch dim
    ...
    #     if self.source_type.stream or self.source_type.from_img:  # batch_size >= 1         # attention
    #         log_string += f'{idx}: '
    #         frame = self.dataset.count
    #     else:
    ...
    #
    #
    #     det = self.results[idx].boxes  # TODO: make boxes inherit from tensors
    #     # print('det:',det)
    #     # print('len of det:',len(det))
    #     if len(det) == 0:
    ...
    #     #         # 在绘制前打印信息
    #     #         print(f'准备绘制目标框和标签，目标类别为: {label}')
    #     #
    #     #         self.annotator.box_label(d.xyxy.squeeze(), label, color=colors(c, True))
    #     #
    #     #         # 在绘制后打印信息
    #     #         print(f'已绘制目标框和标签: {label}')
    #     #     if self.args.save_crop:
    #     #         save_one_box(d.xyxy,
    #     #                      imc,
